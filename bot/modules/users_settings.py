...
async def get_user_settings(from_user, key=None, edit_type=None, edit_mode=None):
    ...
    if key is None:
        ...
        text += f'<b>⚙️ YT-DLP Options:</b> <b><code>{ytopt}</code></b>\n'
        text += f'<b>🔖 Prefix:</b> <code>{prefix}</code>\n'
        text += f'<b>🏷️ Suffix:</b> <code>{suffix}</code>\n'
        ...
        text += f'<b>♻️ Rclone Config:</b> {rccmsg}\n'
        text += f'<b>🛠️ User TD Mode:</b> {tds_mode}'
        ...
        text += f'<b>✂️ Leech Split Size:</b> <code>{split_size}</code>\n'
        text += f'<b>⚔️ Equal Splits:</b> {equal_splits}\n'
